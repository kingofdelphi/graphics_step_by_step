...
void drawHelix(SDL_Surface * screen, const Camera & cam, double height, const Point & p, Uint32 cola, Uint32 colb, double theta = 0) {
	...
	double dy = 10, R = 50, r = 20;
	double HEIGHT_PER_REVOLUTION = 140;
	double dinc = step_angle * HEIGHT_PER_REVOLUTION / (2 * M_PI); // height that will increase when we move step_angle around the helix surface
	for (double y = dinc; y <= height; y += dinc) {
