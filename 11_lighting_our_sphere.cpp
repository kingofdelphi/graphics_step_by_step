...
bool pointInsideTriangle(const Point & p, const Point & a, const Point & b, const Point & c) {
    if (p.y < a.y || p.y > c.y) return 0;
    if (a.x == b.x && p.x == a.x && p.y <= b.y) return 1;
    if (b.x == c.x && p.x == b.x && p.y > b.y) return 1;
    if (a.x == c.x && p.x == a.x) return 1;
    ...
    for (int i = miny; i <= maxy; ++i) {
        ...
        for (int j = max(minx, 0); j <= maxx; ++j) {
            ...
            if (pointInsideTriangle(Point(j, i, 0), pa, pb, pc)) {
                ...
                int xp = j - SCREEN_WIDTH / 2, yp = i - SCREEN_HEIGHT / 2;
                ...
                if (point_z < z_buffer[i][j]) {
                    ...
                    Color dcol(0.2, 0.2, 0.2); // ambient light
...
void drawSphere(SDL_Surface * screen, const Point & center, double radius, const Camera & cam) {
    int phi_steps = 100, theta_steps = 100;
...
int main(int argc, char ** argv) {
    ...
    while (run) {
        ...
        //sphere logic
        ballyvel += gravity;
        sphere_center.x += ballxvel;
        sphere_center.y += ballyvel;
        if (ballyvel > 0) { //moving down
        ...
        clearZBuffer();
        drawSphere(screen, sphere_center, 50, camera);
